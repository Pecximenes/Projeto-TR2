#include <Arduino.h>

#include <SPI.h>
#include <LoRa.h>
#include "Ultrasonic.h"

const int localId = 1;

const int trigPin = 3;
const int echoPin = 4;
Ultrasonic ultrasonic(trigPin, echoPin);


void setup() {
    setupSerial();
    setupLoRa();
}

void setupSerial() {
    Serial.begin(9600);
    while(!Serial);
    Serial.println("Serial iniciada com sucesso");
}

void setupLoRa() {
    if (!LoRa.begin(915E6)) {
        Serial.println("Erro ao iniciar o modulo LoRa");
        while(1);
    }
    Serial.println("Modulo LoRa iniciado com sucesso");
}


void loop() {
    unsigned long startTimeTank = millis();
    bool poolReceived = waitForMode(1);
    if (poolReceived) {
        String tankLevel = readTankLevel();
        sendPacket(0, localId, gatewayId, tankLevel);
        bool sleepReceived = waitForMode(2);
        if (sleepReceived) {
            unsigned long endTimeTank = millis();
            delay(640000 - (endTimeTank - startTimeTank));
        }
    }
}

void sendPacket(int mode, int senderId, int destId, String content="") {
    LoRa.beginPacket();
    LoRa.write(mode);
    LoRa.write(senderId);
    LoRa.write(destId);
    LoRa.write(content.length());
    LoRa.print(content);
}

void waitForMode(int expectedMode) {
    while(1) {
        if (LoRa.parsePacket()) {
            int mode = LoRa.read();
            int gatewayId = LoRa.read();
            int tankId = LoRa.read();
            int contentLength = LoRa.read();

            if (tankId == localId) {
                String content = "";
                while (LoRa.available()) {
                    content += (char)LoRa.read();
                }

                if (mode != expectedMode || contentLength != content.length()) {
                    Serial.println("Pacote corrompido recebido");
                    return false;
                }

                return true;
            }
        }
    }
}


String readTankLevel() {
    int distance = ultrasonic.read();
    return String(distance);
}
